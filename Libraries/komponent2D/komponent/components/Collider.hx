...
typedef HxPolygon = differ.shapes.Polygon;
typedef HxCircle = differ.shapes.Circle;
	...
	*/
	...
	override public function debugDraw()
	{
		...
		for (camera in Screen.cameras)
		{
			...
			Painter.matrix = FastMatrix.fromMatrix3(transform.matrix.multmat(camera.matrix));
	...
	{
		var result = Collision.shapeWithShape(shape, otherCollider.shape);
		if (result != null)
	...
	public function collideWithGroup(otherColliders:Array<Collider>, ignoreItself:Bool = false):Array<CollisionData2D>
	{
		...
		for (collider in otherColliders)
		{
			...
			var result = Collision.shapeWithShape(shape, collider.shape);
			if (result != null)
